...
public class ValidateController {
	...
	public String registProc(
		...
		/*
		메소드의 첫번째 파라미터
			: 폼 값을 커맨드 객체를 통해 한번에 받은 후 어노테이션으로
			모델객체에 저장시 mInfo로 이름을 변경한다.
		두번째 파라미터
			: BindingResult 객체는 validator를 통해 폼 값을 검증한 후 
				해당 결과를 전송받을 객체로, 폼 값에 문제가 있다면 해당
				객체를 통해 확인할 수 있다.
		세번째 파라미터
			: 데이터를 저장할 Model 객체
		 */
		
		//폼 값 검증 완료시 이동할 페이지의 경로
		String viewPage = "03Validate/memberDone";
		/*
		유효성 검증을 위해 정의한 클래스의 객체를 생성한 후 전송된
		폼 값을 저장한 커맨드 객체를 통해 폼 값의 유효성 체크를 실시한다.
