...
지정된 com.kosmo.k11springlegacy를 스캔하여 컨트롤러 클래스를 찾아낸다.
그리고 해당 요청명에 매핑된 메소드를 찾아 요청을 전달하고
비즈니스 로직을 수행한다.
...
@RequestMapping
	: 요청명을 매핑한다. 요청명은 서블릿과 마찬가지로 컨텍스트 루트를
...
public class FormController {
	/*
	폼 값 받기1] 파라미터로 전달되는 값을 HttpServletRequest 객체를
	...
	public String loginMember(HttpServletRequest req, Model model) {
		...
		ViewResolver가 전체경로를 조립하여 웹브랑저에 출력한다.
		 */
		return "01Form/servletRequest";
		
	}
	/*
	폼 값 받기2] @RequestParam 어노테이션으로 폼 값 받기
	...
	public String joinMember(Model model,
		...
		//받아온 폼 값을 각각의 변수에 지정한 후 DTO객체에 세팅함.
	...
	/*
	폼 값 받기3] 커맨드 객체를 이용해서 폼 값 한번에 받기
	조건1 : 쿼리스트링으로 전달되는 파라미터의 갯수와 폼 값을 저장할
		객체(DTO 혹은 VO)의 멤버변수의 갯수가 동일한 것을 권장한다.
	조건2 : 커맨드객체를 사용할 때는 클래스명 앞글자를 소문자로 바꾼 형태의
		매개변수를 사용해야 한다.
		Ex)MemberDTO -> memberDTO
	조건3 : 커맨드객체를 만들 때 파라미터의 갯수는 달라도 상관 없으나
		파라미터 명과 멤버변수 명은 동일해야 하고 이에 해당하는
