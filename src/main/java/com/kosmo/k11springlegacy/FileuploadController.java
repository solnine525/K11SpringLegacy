...
public class FileuploadController {
	...
	/*
	파일업로드 처리
		: 파일업로드는 반드시 POST 방식으로 전송해야 하므로
		컨트롤러 매핑시 method, value 두 가지 속성을 기술한다.
	 */
	...
	public String uploadAction(Model model, MultipartHttpServletRequest req) {
		...
		//폼 값과 파일명을 저장 후 View로 전달하기 위한 맵 컬렉션
		Map returnObj = new HashMap();
		try {
			//업로드 폼의 file 속성의 필드를 얻어온다.
			...
			/*
			물리적 경로를 기반으로 File 객체를 생성한 후 지정된 디렉토리가
			...
			while(itr.hasNext()) {
				...
				file.put("serverFullName",serverFullName); //파일의 전체 경로
