...
차후 supports, validate 두 개의 메소드를 오버라이딩 한다.
 */
public class MemberValidator implements Validator {
	...
	/*
	validate()
	: support()가 true를 반환하는 경우에만 호출되는 메소드로
	실제 폼 값에 대한 검증을 진행한다.
	매개변수1 : 폼값을 저장한 커맨드객체
	매개변수2 : 에러정보를 저장할 Error 타입의 변수
	 */
	@Override
	public void validate(Object obj, Errors errors) {
		...
		/*
		유효성 검증을 이해 전달되는 객체는 커맨드객체가 아니라면
		validate() 메소드 내용 진입 자체가 불가능하므로, 해당 함수에서 "pw"라는 문장은
	...
	//사용자정의 메소드 : 값이 null이거나 길이가 0일 때 false반환
